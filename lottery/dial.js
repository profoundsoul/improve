...
class LotteryDial extends Events {
  constructor(pointer, options) {
    ...
    this.options = Object.assign({
      speed: 30, // 每帧速度
      areaNumber: 8, // 奖区数量
      // 随机结果角度偏差值 为了防止出现指针和扇区分割线无限重合 单位:°
      deviation: 3,
      pointerAngle: 0
    }, options)
    // 指针当前的位置为pointerAngle，补足360度；
    this.complementAngle =this.options.pointerAngle;
    this.pointer = pointer

    this.init();
    this.awardAreaIndex = -1;
  }
  ...
  init() {
    ...
    getPrefix(this.pointer, 'perspective', '1px');
  ...
  reset(event = 'reset') {
    ...
    this._targetAngle = -1
    this.trigger(event)
    if (event === 'reset') getPrefix(this.pointer, this._transform, 'perspective(1px) translate3d(0,0,0) rotate(0deg)')
  }

  setResult(index) {
    this.awardAreaIndex = index;
    // 得到中奖结果 index:中奖奖区下标
    const singleAngle = 360 / this.options.areaNumber // 单个奖区角度值
    // 随机得出结果在扇区内的角度
    let x = Math.floor(Math.random() * 100) %2 ==0 ? 1 : -1;
    let endAngle = x * (Math.random() * singleAngle /4) + singleAngle/2;

    endAngle = Math.max(this.options.deviation, endAngle)
    endAngle = Math.min(singleAngle - this.options.deviation, endAngle)
    endAngle = Math.ceil(endAngle + this.complementAngle - index * singleAngle)
  ...
  step() {
    ...
    // 指针旋转
    getPrefix(this.pointer, this._transform, 'perspective(1px) translate3d(0,0,0) rotate(' + (this._runAngle % 360) + 'deg)')
